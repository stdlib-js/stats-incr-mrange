...
tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrmrange( 3 ), 'function', 'returns expected value' );
	t.end();
});
...
tape( 'the accumulator function correctly handles signed zeros', function test( t ) {
	...
	for ( i = 0; i < data.length; i++ ) {
		v = acc( data[ i ] );
		if ( expected[ i ] === 0.0 ) {
			t.strictEqual( isNegativeZero( v ), isNegativeZero( expected[ i ] ), 'returns expected value for window '+i );
		} else {
